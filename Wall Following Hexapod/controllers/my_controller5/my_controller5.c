...
int main(int argc, char **argv) {
  ...
  WbDeviceTag Beside = wb_robot_get_device("BesideSensor");
  WbDeviceTag Front = wb_robot_get_device("FrontSensor");
  
  wb_distance_sensor_enable(Beside, time_step);
  ...
  while (wb_robot_step(time_step) != -1) {
    ...
    float jarak = wb_distance_sensor_get_value(Beside);
    float jarak1 = wb_distance_sensor_get_value(Front);
    ...
    void arahKanan(){
      ...
      else if (amplitudo >=15 && amplitudo <=22){Kanan=(amplitudo-15)/7;}
    ...
    //printf("Jarak : %f\n", jarak_ukur);
    printf("jarak Samping adalah: %f, ", jarak);
    printf("Depan adalah: %f, ", jarak1);
    //printf("jarak adalah: %f", awal);
    printf("Defuzzyfikasi :%f", defuzyfikasi());
    printf("\n\n");
    ...
    double Tembok_Depan = wb_distance_sensor_get_value(Front) > 90;
